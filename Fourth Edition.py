...
###################Declaration go here#################################
        ...
        self.rollAccel    = []
        self.pitchAccel   = []
        self.sliderVal1   = [1000]
        self.sliderVal2   = [1000]
        self.sliderVal3   = [1000]
        self.sliderVal4   = [1000]
        self.sliderVal5   = [1000]
        self.DLogButton   = []
#######################################################################
...
##
        ...
        self.timer_7.timeout.connect(self.SURGE)
        self.timer_7.start()
###################################################################################################
###################Function go here################################################################
###################################################################################################
    ...
    def FORWARD(self):
       if self.Lstartbutton:
          if self.sliderVal1[0] != 2000:
             self.sliderVal1[0] += 10
             self.verticalSlider.setValue(self.sliderVal1[0]+10)
             
    def BACKWARD(self):
       if self.Lstartbutton:
          if self.sliderVal1[0] != 1000:
             self.sliderVal1[0] -= 10
             self.verticalSlider.setValue(self.sliderVal1[0]-10)

    def SURGE(self):
       global fileName
       if not self.firstRun11:
           ...
           fileName  = 'HEAVEDATA'+'_'+str(year)+'_'+str(month)+'_'+str(day)+'_'+str(hour)+'_'+str(minute)+'_'+str(second)
           self.firstRun11.append(1)
       TimeTaken      = self.timeStamp()
       val = self.verticalSlider.value()
       pi.set_servo_pulsewidth(ESC_GPIO, val)
       pi.set_servo_pulsewidth(ESC_GPIO2, val)
       val1 = self.horizontalSlider_4.value()
       pi.set_servo_pulsewidth(ESC_GPIO1,val1)
       with open(fileName+ '.txt','a') as file:
           file.write(str(TimeTaken)+','+str(val1)+'\n')
    ...
    def YAWRIGHT(self):
       if self.Lstartbutton:
          if self.sliderVal2[0] != 2000:
             self.sliderVal2[0] += 1
             self.horizontalSlider.setValue(self.sliderVal2[0]+10)

    def YAWLEFT(self):
       if self.Lstartbutton:
          if self.sliderVal2[0] != 1000:
             self.sliderVal2[0] -= 10
             self.horizontalSlider.setValue(self.sliderVal2[0]-10)

    def PITCHDOWN(self):
       if self.Lstartbutton:
          if self.sliderVal3[0] != 2000:
             self.sliderVal3[0] += 10
             self.horizontalSlider_2.setValue(self.sliderVal3[0]+10)

    def PITCHUP(self):
       if self.Lstartbutton:
          if self.sliderVal3[0] != 1000:
             self.sliderVal3[0] -= 10
             self.horizontalSlider_2.setValue(self.sliderVal3[0]-10)

    def ROLLCW(self):
       if self.Lstartbutton:
          if self.sliderVal4[0] != 2000:
             self.sliderVal4[0] += 10
             self.horizontalSlider_3.setValue(self.sliderVal4[0]+10)

    def ROLLCCW(self):
       if self.Lstartbutton:
          if self.sliderVal4[0] != 1000:
             self.sliderVal4[0] -= 10
             self.horizontalSlider_3.setValue(self.sliderVal4[0]-10)

    def HEAVEUP(self):
       if self.Lstartbutton:
          if self.sliderVal5[0] != 2000:
             self.sliderVal5[0] += 10
             self.horizontalSlider_4.setValue(self.sliderVal5[0]+10)

    def HEAVEDOWN(self):
       if self.Lstartbutton:
          if self.sliderVal5[0] != 1000:
             self.sliderVal5[0] -= 10
             self.horizontalSlider_4.setValue(self.sliderVal5[0]-10)
    ...
    def timeStamp(self):
        global timestamp
        if not self.firstRun10:
            timestamp = time.time()
            self.firstRun10.append(1)
